...
#define SDIO_TRANSFER_CLK_DIV    0x20  //If communication is abnormal, it is recommended to increase this value.
...
typedef struct
{
  SD_CSD SD_csd;//SD卡的CSD寄存器信�??
  SD_CID SD_cid;//SD卡的CID寄存器信�??
  long long CardCapacity;//SD卡容量，单位为字�??
  u32 CardBlockSize;//SD卡块大小，单位为字节（通常512字节�??
  u16 RCA;//SD卡的相对地址，主机分配给卡的唯一地址，用于区分总线上的多张�??
  u8 CardType;//SD卡类�??
} SD_CardInfo;
