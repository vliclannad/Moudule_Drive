...
SD_Error SD_Init( void )
{
    ...
    errorstatus = SD_PowerON();//查询卡的工作电压和时钟控制配置
    //此时SD卡处于识别模式下的准备状态
    if( errorstatus == SD_OK )
    {
        errorstatus = SD_InitializeCards();//进行与SD卡相关的初始化，卡进入数据传输模式下的待机模式
    }

    if( errorstatus == SD_OK )
    {
        errorstatus = SD_GetCardInfo( &SDCardInfo );//获取SD卡信息，主要包括CID和CSD寄存器内容
    }

    if( errorstatus == SD_OK )
    {
        errorstatus = SD_SelectDeselect( ( u32 )( SDCardInfo.RCA << 16 ) );//选择特定RCA的SD卡，卡从待机转变为传输模式
    }

    if( errorstatus == SD_OK )
    {
        errorstatus = SD_EnableWideBusOperation( 1 );//拓展数据线宽度，但这里还是使用1根线
    }
...
SD_Error SD_PowerON( void )
{
    ...
    SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;//时钟分频系数，CLK时钟频率=SDIOCLK/(CLKDIV+2)，卡识别阶段时钟不能超过400KHz
    SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;//CLK引脚时钟有效沿（一般高电平）
    SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;//时钟分频旁路使用，使能SDIOCLK直接驱动CLK线输出时钟，禁用则经过分频再输出到CLK
    SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;//节能模式选择，使能CLK线在总线激活是才有时钟输出；禁用则始终输出时钟
    SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;//数据线宽度选择，可选1位，4位，8位，默认使用1位数据总线
    ...
    for( i = 0; i < 74; i++ ){
        ...
        SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;//等待使能
        SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;//命令路径状态机
    ...
    //发送CMD8命令，检测SD卡支持的操作条件，主要是电压匹配，响应类型R7
    ...
    //发送ACMD41判断卡的类型前，需要线发送CMD55,响应类型R1
    ...
    if( errorstatus == SD_OK )//判断是否有响应
    {
        ...
        {
            CardType = SDIO_HIGH_CAPACITY_SD_CARD;//判断卡类型
        }
    }
    else
    {
